...
class Doctor:
    ...
    def add_check(
        self, name: str, passed: bool, message: str = "", level: str = "error"
    ):
        """Add a check result"""
    ...
    def check_config(self) -> bool:
        ...
        try:
            if not self.config_manager.config_exists():
                self.add_check(
                    _("doctor.check.config"), False, _("doctor.config.not_found")
                )
                return False
            ...
            if not config.username:
                self.add_check(
                    _("doctor.check.username"), False, _("doctor.config.no_username")
                )
                return False
            ...
            self.add_check(_("doctor.check.config"), True, _("doctor.config.valid"))
            return True

        except Exception as e:
            self.add_check(_("doctor.check.config"), False, str(e))
            return False
    ...
    def check_password(self) -> bool:
        ...
        try:
            ...
            if password:
                self.add_check(
                    _("doctor.check.password"), True, _("doctor.password.found")
                )
                return True
            else:
                ...
                if os.environ.get("CPOLAR_PASSWORD"):
                    self.add_check(
                        _("doctor.check.password"), True, _("doctor.password.env")
                    )
                    return True
                else:
                    self.add_check(
            ...
            self.add_check(_("doctor.check.password"), False, str(e))
            return False
    ...
    def check_network(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.add_check(_("doctor.check.network"), True, _("doctor.network.ok"))
                return True
            else:
                self.add_check(
            ...
            self.add_check(
                _("doctor.check.network"), False, _("doctor.network.timeout")
            )
            return False
        except Exception as e:
            self.add_check(
    ...
    def check_cpolar_auth(self) -> bool:
        ...
        try:
            ...
            self.add_check(
                _("doctor.check.cpolar_auth"), True, _("doctor.cpolar.auth_success")
            )
            ...
            self.add_check(_("doctor.check.cpolar_auth"), False, str(e))
            return False
        ...
        try:
            ...
            if ssh_key_path.exists():
                ...
                if mode != "600":
                    ...
                    self.add_check(
                        _("doctor.check.ssh_key"), True, _("doctor.ssh.key_exists")
                    )
                ...
                self.add_check(
                    _("doctor.check.ssh_key"), True, _("doctor.ssh.key_will_create")
                )
                return True

        except Exception as e:
            self.add_check(_("doctor.check.ssh_key"), False, str(e))
            return False
    ...
    def check_ssh_config(self) -> bool:
        ...
        try:
            ...
            self.add_check(_("doctor.check.ssh_config"), False, str(e))
            return False
    ...
    def check_command(self, command: str, name: str) -> bool:
        ...
        try:
            if shutil.which(command):
                self.add_check(name, True, _("doctor.command.found", command=command))
                return True
            else:
                self.add_check(
                    name,
            ...
            self.add_check(name, False, str(e))
            return False
    ...
    def display_results(self):
        ...
        table = Table(
            title=_("doctor.title"), show_header=True, header_style="bold magenta"
        )
        ...
        if self.has_error or self.has_warning:
            ...
            for name, passed, message, level in self.checks:
                if not passed:
                    if (
                        _("doctor.check.config") in name
                        and "config" not in shown_recommendations
                    ):
                        console.print(
                            f"  [cyan]•[/cyan] {_('doctor.recommend.run_init')}"
                        )
                        console.print(f"    [dim]{_('doctor.cmd.init')}[/dim]")
                        shown_recommendations.add("config")
                    elif (
                        _("doctor.check.password") in name
                        and "password" not in shown_recommendations
                    ):
                        console.print(
                            f"  [cyan]•[/cyan] {_('doctor.recommend.set_password')}"
                        )
                        ...
                        if platform.system() == "Windows":
                            console.print(
                                f"    [dim]{_('doctor.cmd.option1')} {_('doctor.cmd.password.win')}[/dim]"
                            )
                        else:
                            console.print(
                                f"    [dim]{_('doctor.cmd.option1')} {_('doctor.cmd.password.unix')}[/dim]"
                            )
                        console.print(
                            f"    [dim]{_('doctor.cmd.option2')} {_('doctor.cmd.password.save')}[/dim]"
                        )
                        shown_recommendations.add("password")
                    elif (
                        _("doctor.check.network") in name
                        and "network" not in shown_recommendations
                    ):
                        console.print(
                            f"  [cyan]•[/cyan] {_('doctor.recommend.check_network')}"
                        )
                        ...
                        _("doctor.check.cpolar_auth") in name
                        and "auth" not in shown_recommendations
                    ):
                        console.print(
                            f"  [cyan]•[/cyan] {_('doctor.recommend.check_credentials')}"
                        )
                        ...
                        _("doctor.check.tunnel") in name
                        and "tunnel" not in shown_recommendations
                    ):
                        console.print(
                            f"  [cyan]•[/cyan] {_('doctor.recommend.check_server')}"
                        )
    ...
    def run(self) -> bool:
        ...
        if self.config_manager.config_exists():
            ...
            if self.config_manager.get_config() and (
                self.config_manager.get_password(
                    self.config_manager.get_config().username
                )
                or os.environ.get("CPOLAR_PASSWORD")
            ):
